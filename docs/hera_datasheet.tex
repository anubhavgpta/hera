...
\begin{minipage}[t]{0.38\linewidth}
  \raggedleft
  \includegraphics[height=1.05cm]{circle_logo}
\end{minipage}

\vspace{22mm}

% Hera wordmark
\noindent\includegraphics[width=9.5cm]{hera_logo}
...
\begin{tabular}{L{4.1cm} L{3.2cm} L{4.1cm} L{2.8cm}}
\rowcolor{tabhead}
...
\portname{wr\_k\_data}    & in  & 1024 & Key vector (\texttt{HEAD\_DIM} $\times$ \texttt{DATA\_WIDTH} bits). \\
\rowcolor{tabalt}
\portname{wr\_v\_data}    & in  & 1024 & Value vector (\texttt{HEAD\_DIM} $\times$ \texttt{DATA\_WIDTH} bits). \\
...
\begin{tabularx}{\linewidth}{C{1.6cm} L{4.2cm} C{1.3cm} C{1.3cm} X}
\toprule
